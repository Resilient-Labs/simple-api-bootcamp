    ...
    .then(response => {
      ...
      response.forEach((el, index)=>{
        let catLen = cat.length;
            // to get it to wrap back so the index's will match up so you will always recieve an image
        let indexCatUrl = index % catLen;
        let images = document.createElement("img");
        images.src=cat[indexCatUrl];
